...
const Input = (props) => {
	const { leftElement, rightElement, leftTitle,
		leftTitleBox, leftTextStyle, placeholderTextColor,
		disabled, style, pointerEvents, opacity } = props;
	if (leftTitle || leftElement || rightElement) {
		return (
			<View style={[styles.leftTitleBox, leftTitleBox]}>
				{leftElement ? leftElement : <Text style={[styles.leftTextStyle, leftTextStyle]}>{leftTitle}</Text>}
				<TextInput
