...
const GenerateQRCode = () => {
  ...
  const rightElement = useMemo(() => (
    <Touchable style={{ padding: 5 }}
      ...
      <TextM style={styles.rightStyle}>{i18n.t('login.backupQRCode.later')}</TextM>
    </Touchable>
  ), []);

  return (
    <View style={Gstyle.container}>
      <CommonHeader title={i18n.t('login.backupQRCode.title')} canBack rightElement={rightElement} />
      ...
      <CommonButton title={i18n.t('login.backupQRCode.saveQRCode')} onPress={() => screenshots(viewShot)} />
    </View>
  );
}
