...
module.exports.getContract = function(
  aelf,
  wallet,
  contractNameAddressKeyValues,
) {
  const contractInstances = {};
  const {contractAddresses} = config;
  const promise = Object.entries(contractNameAddressKeyValues).map(
  ...
  const otherPromise = Object.entries(contractAddresses).map(
    ([contractName, contractAdress]) => {
      return aelf.chain
        .contractAt(contractAdress, wallet)
        .then(contractInstance => {
...
module.exports.getOtherContracts = function(aelf, wallet) {
  const {contractAddresses} = config;
  const contractInstances = {};
  const promise = Object.entries(contractAddresses).map(
    ([contractName, contractAdress]) => {
      return aelf.chain
        .contractAt(contractAdress, wallet)
        .then(contractInstance => {
